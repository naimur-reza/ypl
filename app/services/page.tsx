...
async function getServices() {
  await connectDB();
  const hiddenServiceTitles = ["release status", "lifecycle"];
  const services = await Service.find({ isActive: true })
    .sort({ order: 1 })
    .lean();
  return JSON.parse(JSON.stringify(services)).filter(
    (service: any) => {
      const title = String(service.title).trim().toLowerCase();
      return !hiddenServiceTitles.some((hiddenTitle) =>
        title.includes(hiddenTitle),
      );
    },
  );
}
