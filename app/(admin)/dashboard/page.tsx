...
export default function DashboardPage() {
  ...
  const filteredCvs = useMemo(() => {
    return cvs.filter((cv) => {
      ...
      if (!dateVal || Number.isNaN(dateVal.getTime())) return true;
  ...
  const totalCvs = filteredCvs.length;
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-lg border border-border/60 bg-card p-3">
        ...
        {(dateFrom || dateTo) && (
          <button
            ...
            className="h-9 rounded-md px-3 text-sm text-muted-foreground hover:text-foreground transition-colors"
          >
            Clear
          </button>
        )}
              ...
              <CardContent className="pt-4">
                <ChartContainer config={emptyChartConfig} className="h-[280px] w-full">
                  <LineChart data={submissionsByMonth} margin={{ left: 0, right: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} className="stroke-border/60" />
                    <XAxis dataKey="month" tickLine={false} axisLine={false} tickMargin={10} minTickGap={40} />
                    <YAxis tickLine={false} axisLine={false} tickMargin={8} allowDecimals={false} />
                    <ChartTooltip
                      content={
                        <ChartTooltipContent
              ...
              <CardContent className="pt-4">
                <ChartContainer config={emptyChartConfig} className="h-[280px] w-full">
                  <BarChart data={cvExperienceDistribution} margin={{ left: 0, right: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} className="stroke-border/60" />
                    <XAxis dataKey="band" tickLine={false} axisLine={false} tickMargin={10} minTickGap={40} />
                    <YAxis tickLine={false} axisLine={false} tickMargin={8} allowDecimals={false} />
                    <ChartTooltip
                      content={
                        <ChartTooltipContent
